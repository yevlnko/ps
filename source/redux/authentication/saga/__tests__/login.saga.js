...
describe('login saga:', () => {
    ...
    test('should handle !== 200', () => {
        ...
        expect(
            clone.next(__.fetchResponseFail).value
        ).toEqual(
            call([__.fetchResponseFail, __.fetchResponseFail.json]),
        );

        expect(
            clone.next(__.responseDataFail).value
        ).toEqual(
    ...
    test('fetch request should complete successfully', () => {
        expect(
            saga.next(__.fetchResponseSuccess).value
        ).toEqual(
    ...
    test('localStorage should contain a token', () => {
        expect(
            saga.next(__.responseDataSuccess).value
        ).toBeUndefined();
